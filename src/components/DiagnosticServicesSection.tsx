...
export const DiagnosticServicesSection: React.FC = () => {
  ...
  return (
    <section className="py-12 bg-slate-50 dark:bg-[#0F0F14] text-slate-900 dark:text-slate-100 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        <div className="bg-white dark:bg-[#181824] rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-amber-500/20 shadow-xs">
          <div className="flex items-center gap-2 mb-2">
            <Activity className="w-5 h-5 text-amber-500 dark:text-amber-400" />
            <h3 className="text-xs font-bold text-amber-600 dark:text-amber-400 uppercase tracking-widest">Doorstep Health Services</h3>
          </div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-300 max-w-2xl mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {packages.map((pkg) => (
              <div key={pkg.id} className="bg-slate-50 dark:bg-[#181824] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 hover:border-amber-500/40 transition-all flex flex-col justify-between shadow-xs">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[10px] font-bold bg-amber-100 dark:bg-amber-500/10 text-amber-800 dark:text-amber-400 px-2.5 py-1 rounded-full uppercase border border-amber-300 dark:border-amber-500/30">
                      NABL Accredited Lab
                    </span>
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold">{pkg.duration}</span>
                  </div>
