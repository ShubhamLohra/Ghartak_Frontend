...
export const Navbar: React.FC<NavbarProps> = ({ onSearch }) => {
  ...
  return (
    <>
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
                  ...
                  {cities.map((city) => (
                    <button
                      ...
                      className={`w-full text-left px-3.5 py-2 text-xs font-medium flex items-center justify-between hover:bg-indigo-50 hover:text-indigo-700 transition-colors ${
                        selectedCity === city ? 'text-indigo-600 font-bold bg-indigo-50/60' : 'text-slate-700'
                      }`}
                    >
                      <span>{city}</span>
                      {selectedCity === city && <CheckCircle2 className="w-3.5 h-3.5 text-indigo-600" />}
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="relative p-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold transition-all tap-target"
              title="Cart Summary"
            >
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-200 shadow-lg px-2 py-1.5">
