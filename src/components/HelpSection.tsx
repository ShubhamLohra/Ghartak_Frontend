  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 mb-24">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">How can we help you?</h1>
        <p className="text-slate-600 mt-1 text-sm sm:text-base">We are here to support you 7 days a week from 8:00 AM to 9:00 PM</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
        <a
          ...
          className="consumer-card-interactive p-5 flex items-center gap-4 bg-emerald-500/10 border-emerald-200 text-emerald-800 hover:border-emerald-400"
        >
          <div className="p-3 bg-emerald-500 text-white rounded-xl">
            <MessageSquare className="w-6 h-6" />
          </div>
          <div>
            <h3 className="font-semibold text-base text-slate-900">Chat on WhatsApp</h3>
            <p className="text-xs text-slate-600">Quick response within 5 minutes</p>
          </div>
        </a>

        <a
          href="tel:18001234567"
          className="consumer-card-interactive p-5 flex items-center gap-4 bg-indigo-500/10 border-indigo-200 text-indigo-800 hover:border-indigo-400"
        >
          <div className="p-3 bg-indigo-600 text-white rounded-xl">
            <PhoneCall className="w-6 h-6" />
          </div>
          <div>
            <h3 className="font-semibold text-base text-slate-900">Call Support</h3>
            <p className="text-xs text-slate-600">Toll-free 1800-123-4567</p>
          </div>
        </a>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-start gap-3">
          ...
          <div>
            <h4 className="text-xs font-semibold text-slate-900 uppercase tracking-wide">100% Verified</h4>
            <p className="text-xs text-slate-600 mt-0.5">Background checked professionals</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-start gap-3">
          <Clock className="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" />
          <div>
            <h4 className="text-xs font-semibold text-slate-900 uppercase tracking-wide">On-Time Service</h4>
            <p className="text-xs text-slate-600 mt-0.5">Prompt arrival at chosen time slot</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-start gap-3">
          <CreditCard className="w-5 h-5 text-emerald-600 flex-shrink-0 mt-0.5" />
          <div>
            <h4 className="text-xs font-semibold text-slate-900 uppercase tracking-wide">Transparent Pricing</h4>
            <p className="text-xs text-slate-600 mt-0.5">No hidden charges or extra fees</p>
          </div>
        </div>
      </div>
          ...
          {faqs.map((faq, index) => (
            <div key={index} className="py-4">
              <button
                ...
                className="w-full flex items-center justify-between text-left font-medium text-slate-900 text-sm sm:text-base py-1 hover:text-indigo-600 transition-colors"
              >
                <span>{faq.question}</span>
                <ChevronDown
                  className={`w-5 h-5 text-slate-400 transition-transform ${
                    openFaq === index ? 'transform rotate-180 text-indigo-600' : ''
                  }`}
                />
              </button>
              {openFaq === index && (
