...
export const RawMaterialMarketplace: React.FC = () => {
  ...
  return (
    <section className="py-12 bg-slate-50 dark:bg-[#0F0F14] text-slate-900 dark:text-slate-100 min-h-screen transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="text-xs font-bold text-amber-600 dark:text-amber-400 uppercase tracking-widest">Ghar Tak Hardware Supply Depot</h3>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-4 md:mt-0">
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${selectedCategory === cat ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/25' : 'bg-white dark:bg-[#181824] text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-[#222232] border border-slate-200 dark:border-slate-700/60'}`}
              >
                {cat}
              </button>
            ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMaterials.map((item) => (
            <div key={item.id} className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 hover:border-amber-500/40 transition-all flex flex-col justify-between shadow-xs">
              
              <div>
                <div className="relative h-44 rounded-xl overflow-hidden mb-4 bg-slate-100 dark:bg-slate-800">
                  <img 
                    src={item.imageUrl} 
                    alt={item.name} 
                    onError={(e) => {
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-1 text-xs text-amber-500 dark:text-amber-400 font-bold">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-2 mb-4">{item.description}</p>
              </div>
